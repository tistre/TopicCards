...
class XtmImport
{
    ...
    public function __construct(\TopicCards\iTopicMap $topicmap)
    {
    ...
    protected function importTypes(\DOMElement $context_node, \TopicCards\iTopic $topic)
    {
        $topic_refs = [ ];
    ...
    protected function importSubjectIdentifiers(\DOMElement $context_node, \TopicCards\iTopic $topic)
    {
        $this->importSubjects('subjectIdentifier', $context_node, $topic);
    }
    
    
    protected function importSubjectLocators(\DOMElement $context_node, \TopicCards\iTopic $topic)
    {
        $this->importSubjects('subjectLocator', $context_node, $topic);
    }
    
    
    protected function importSubjects($what, \DOMElement $context_node, \TopicCards\iTopic $topic)
    {
        $hrefs = [ ];
    ...
    protected function importNames(\DOMElement $context_node, \TopicCards\iTopic $topic)
    {
        $names = [ ];
    ...
    protected function importOccurrences(\DOMElement $context_node, \TopicCards\iTopic $topic)
    {
        $occurrences = [ ];
        ...
        foreach ($context_node->getElementsByTagName('occurrence') as $node)
        {
            ...
            foreach ($node->getElementsByTagName('resourceData') as $subnode)
            {
                ...
                $value = \TopicCards\Utils\DatatypeUtils::getValueFromDomNode($subnode, $datatype);
    ...
    protected function importRoles(\DOMElement $context_node, \TopicCards\iAssociation $association)
    {
        $roles = [ ];
